...
#' according to the auxiliary information given, see Details above.}
#'
...
setMethod(
  ...
  function(object) {
    ...
    if (!is.null(slot(object, "cluster"))) { # cluster sampling
      ...
      Y_c <- tapply(
        slot(object, "data")[, predictand] * as.numeric(include),
    ...
    for (group in sort(unique(slot(object, "data")[, smallArea])[!is.na(unique(slot(object, "data")[, smallArea]))]))
    {
      if (!is.null(slot(object, "cluster"))) { # cluster sampling
        ...
        G <- slot(object, "data")[, smallArea] == group
        G[is.na(G)] <- FALSE
        result <- data.frame(
          smallArea = group,
          prediction = mean(Y[G]),
  ...
  function(object) {
      ...
      if (VIMFOLD) { ## commons
        if (is.null(slot(object, "s1"))) {
          ...
          s1c <- rep(
            ...
            length(unique(
              slot(object, "data")[
                !is.na(slot(object, "data")[, slot(object, "cluster")]),
                slot(object, "cluster")
              ]
            ))
          )
        ...
        if (length(predictors) == 1) {
          ...
          Z_c <- mapply(append,
            mapply("/",
              by(slot(object, "data")[, c(predictors)] * as.numeric(include),
                slot(object, "data")[, slot(object, "cluster")], colSums,
                simplify = TRUE
              ),
              M,
              SIMPLIFY = FALSE
    ...
    for (group in sort(unique(slot(object, "data")[, smallArea])[!is.na(unique(slot(object, "data")[, smallArea]))]))
    {
          ...
          "partially" = {
            ...
            tmZ1_G <- as.numeric(
              cbind(
                1,
                subset(
                  slot(object, "smallAreaMeans")[, predictors1],
      ...
      if (!is.null(slot(object, "cluster"))) { # cluster sampling
        ...
        if (VIMFOLD) { # commons
          ## extended model
          tmp <- slot(object, "data")[, smallArea] == group
          tmp[is.na(tmp)] <- FALSE
          ...
          , "partially" = {
            ...
            if (length(predictors1) == 1) {
              ...
              Z1_c <- mapply(append,
                mapply("/",
                  tapply(
                    slot(object, "data")[, predictors1] * as.numeric(include),
                    slot(object, "data")[, slot(object, "cluster")], sum
                  ),
                  M,
                  SIMPLIFY = FALSE
          ...
          "exhaustive" = {
            tmZ_Gc <- as.numeric(
              cbind(
                1,
                subset(
                  slot(object, "smallAreaMeans")[, predictors],
                  slot(object, "smallAreaMeans")[, smallArea] == group
                )
              )
        ...
        switch(pred.type # uncommons
          ...
          "non-exhaustive" = {
            a2.35 <- a2.34 %*% b2.28
            a2.37 <- Reduce("+", lapply(
              lapply(eZ[s1 & G], function(x) x - a2.34),
              function(x) as.numeric(x) %o% as.numeric(x)
            )) / (n_s1G * (n_s1G - 1))
